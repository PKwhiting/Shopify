...
class Product:
    ...
    @classmethod
    def search(cls, client: 'ShopifyClient', query: str = "", 
               first: int = 10, after: Optional[str] = None,
               filters: Optional[Dict[str, Any]] = None) -> List['Product']:
        """
        Search for products.
    ...
    def get(cls, client: 'ShopifyClient', product_id: str) -> Optional['Product']:
        ...
        if result and 'data' in result and 'product' in result['data'] and result['data']['product']:
    ...
    def get_by_handle(cls, client: 'ShopifyClient', handle: str) -> Optional['Product']:
        ...
        if result and 'data' in result and 'productByHandle' in result['data'] and result['data']['productByHandle']:
    ...
    def __repr__(self) -> str:
        ...
        return f"Product(id='{self.id}', title='{self.title}', handle='{self.handle}', status='{self.status}')"
